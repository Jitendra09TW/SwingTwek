...
//        drawingView.userSettings.strokeWidth = strokeWidths[2]
    ...
    func dragSlider(seekSlider: UISlider) {
        self.player?.pause()
        //self.smoothSlider(seekSlider: seekSlider)
    ...
    func smoothSlider(seekSlider: UISlider) {
...
extension CreateTweakViewController {
    ...
    func stepByFrame(in direction: Direction, slider:UISlider) {
        ...
        let seconds = Double(10) / Double(frameRate)
        ...
        print("TimeScale\(timescale)") //Double(slider.value)
        print("FrameRate\(frameRate)")
        print("SliderContinuous",playBackSlider.isContinuous)
        ...
        //self.playBackSlider.value = slider.value
    }
